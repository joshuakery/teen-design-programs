...
export const getSelectOptions = deserialized => {
    const selectOptions = {
      category: [],
      state: [],
      city: [],
...
function fetchCsv() {
    return fetch('/data/teen-design-programs.csv').then(function (response) {
...
function setData(result, setDataCallBack) {
    // result.data.shift(); //don't include the headers row
    // this.setSelectOptions(result.data);
    const deserialized = deserializeData(result.data);
    console.log(deserialized);
    const selectOptions = getSelectOptions(deserialized);
    const state = {
        data: deserialized,
        filtered: deserialized,
        select: {
            category: '',
            state: '',
            city: '',
    ...
    setDataCallBack(state);
}

export async function getCsvData(setDataCallback) {
    let csvData = await fetchCsv();

    Papa.parse(csvData, {
        complete: function(results, file) {
            setData(results,setDataCallback);
        }
    });
}
